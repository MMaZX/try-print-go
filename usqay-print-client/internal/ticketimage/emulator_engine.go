...
func NewEngine(config Config) (*Engine, error) {
	...
	// Ambos slots (Font A/B, el equivalente emulado a las dos fuentes de
	// firmware de una térmica real) cargan la misma Cascadia Code Mono: acá
	// no hay negrita por archivo separado, el motor dibuja "bold" pintando
	// el carácter dos veces con un offset (ver render_text.go) — no depende
	// de qué .ttf esté cargado en cada slot.
	const cascadiaCode = "CaskaydiaCoveNerdFontMono-Regular.ttf"
	fontAWidth, fontAHeight := config.FontAWidth, config.FontAHeight
	if fontAWidth <= 0 {
		fontAWidth = constants.FontAWidth
	}
	if fontAHeight <= 0 {
		fontAHeight = constants.FontAHeight
	}
	fontBWidth, fontBHeight := config.FontBWidth, config.FontBHeight
	if fontBWidth <= 0 {
		fontBWidth = constants.FontBWidth
	}
	if fontBHeight <= 0 {
		fontBHeight = constants.FontBHeight
	}
	errA := fonts.LoadFont("A", cascadiaCode, fontAWidth, fontAHeight)
	errB := fonts.LoadFont("B", cascadiaCode, fontBWidth, fontBHeight)
	if errA != nil {
...
// PrintSegmentAt draws text at an explicit x offset on the current line and
// returns the x position right after it, so consecutive segments with
// distinto estilo (negrita/tamaño por celda) se concatenan en la misma fila.
// Agregado local sobre el vendor, ver TextRenderer.RenderTextAt.
func (e *Engine) PrintSegmentAt(text string, x float64) float64 {
	return e.textRenderer.RenderTextAt(text, x)
}
