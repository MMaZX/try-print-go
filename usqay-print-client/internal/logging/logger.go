// Package logging provides a daily-rotating log writer with monthly archival.
package logging
...
// DailyWriter is an io.Writer that writes to logs/YYYY-MM-DD.log,
// automatically rolling over to a new file at midnight.
...
// NewDailyWriter creates logDir if necessary and opens today's log file.
...
func ArchivePreviousMonth(logDir string) error {
	...
	if err != nil {
		return fmt.Errorf("leer directorio de logs: %w", err)
	}
	...
	if err := os.MkdirAll(zipDir, 0o755); err != nil {
		return fmt.Errorf("crear directorio de archivo: %w", err)
	}

	zipPath := filepath.Join(zipDir, fmt.Sprintf("%s-%s.zip", year, monthName))
	if err := createZip(zipPath, toArchive); err != nil {
		return fmt.Errorf("crear zip %s: %w", zipPath, err)
	}

	var removeErrors []string
	for _, f := range toArchive {
		if err := os.Remove(f); err != nil {
			removeErrors = append(removeErrors, fmt.Sprintf("%s: %v", filepath.Base(f), err))
		}
	}
	if len(removeErrors) > 0 {
		// Non-fatal: archive exists, but originals couldn't be removed.
		return fmt.Errorf("archivado exitoso pero no se pudo eliminar: %s", strings.Join(removeErrors, "; "))
	}
	return nil
}

// ArchiveResult carries the outcome of an archive operation so the caller can log it.
type ArchiveResult struct {
	ZipPath  string
	Archived int
}

// ArchivePreviousMonthResult is like ArchivePreviousMonth but also returns metadata.
func ArchivePreviousMonthResult(logDir string) (*ArchiveResult, error) {
	prev := time.Now().AddDate(0, -1, 0)
	year := prev.Format("2006")
	monthName := spanishMonth(prev.Month())
	prefix := prev.Format("2006-01")
	...
	for _, f := range toArchive {
		_ = os.Remove(f) // best-effort; zip already contains the data
	}
